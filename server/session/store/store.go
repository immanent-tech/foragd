...
// FindCtx should return the data for a session token from the store. If the session token is not found or is expired,
// the found return value should be false (and the err return value should be nil). Similarly, tampered or malformed
...
// AllCtx should return a map containing data for all active sessions (i.e. sessions which have not expired). The map
// key should be the session token and the map value should be the session data. If no active sessions exist this should
