...
func HandleRTDN(appCfg *config.AppConfig, userSvc UserService) http.HandlerFunc {
	return func(res http.ResponseWriter, req *http.Request) {
		...
		webhookURL := appCfg.GetBaseURL().JoinPath("/webhooks/googleplay")
		...
		if appCfg.IsProduction() {
			if err := validateNotification(req, webhookURL); err != nil {
		...
		if sn := notification.SubscriptionNotification; sn != nil {
			...
			user, err := userSvc.GetUserByPurchaseToken(req.Context(), sn.PurchaseToken)
			if err != nil {
				...
				if purchase.LinkedPurchaseToken != "" {
					user, err = userSvc.GetUserByPurchaseToken(req.Context(), purchase.LinkedPurchaseToken)
				}
				if err != nil {
			...
			case !isGrantableState(purchase.SubscriptionState):
				...
				if err := revokeSubscription(req.Context(), userSvc, user); err != nil {
			...
			default:
				slogctx.Debug(req.Context(), "Updating subscription entitlement.",
					slog.String("sku", subscription.SKU),
					slog.String("state", purchase.SubscriptionState))
				if err := updateSubscription(req.Context(), userSvc, user, purchase, sn.PurchaseToken); err != nil {
...
func validateNotification(req *http.Request, path *url.URL) error {
	...
	payload, err := idtoken.Validate(req.Context(), token, path.String())
	if err != nil {
