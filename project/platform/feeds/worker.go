...
type dbAPI interface {
	GetUpdatedFeeds(since time.Time) ([]Feed, error)
}

type cacheAPI interface {
	CacheFeedItems(itemCh chan FeedItem)
}
...
func (w *GetFeedsWorker) Execute(ctx context.Context, db dbAPI, cache cacheAPI) (int, error) {
	...
	for _, feed := range newFeeds {
		...
		if err := scheduler.Schedule(functionJobDetail, quartz.NewSimpleTrigger(time.Minute)); err != nil {
...
func newFeedWorker(feed *Feed) *FeedWorker {
	return &FeedWorker{
		feed: feed,
	}
}
